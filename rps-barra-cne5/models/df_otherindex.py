...
class DfOtherindex(BaseModel):
    """
    指数收盘价（上证指数、沪深300、中证500）
    数据特性：交易日有数据。取数范围：过去252交易日滚动
    """

    table_name = 'df_otherindex'
    columns = ['s_info_windcode', 'trade_dt', 's_dq_close']
    model_type = ModelEnum.MARKET_DATA

    def create(self):
        g.ck.command("""
        create table if not exists df_otherindex
        (
    ...
    def query_db(self, start, end) -> DataFrame:
        return SourcesDataDB().df_otherindex(start, end)
